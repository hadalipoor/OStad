...
public:
    ...
    String handlecreate(int ModuleId, int pinNumber, int DryTreshold, int WetTreshold, String Type);
    String handleupdate(int id, int ModuleId, int pinNumber, int DryTreshold, int WetTreshold, String Type);
...
SoilMoistureApis::SoilMoistureApis(Context* cntxt, bool add_apis): context(cntxt) {
    ...
    context->getNetwork()->addApi(new ResourceNode(std::string(class_path + "/create"), LambdaResourceNode::REQUEST_METHOD_POST, [&](HTTPRequest * req, HTTPResponse * res) {
        ...
        if (!req->getParams()->isQueryParameterSet("ModuleId") || !req->getParams()->isQueryParameterSet("PinNumber") || !req->getParams()->isQueryParameterSet("DryTreshold") || !req->getParams()->isQueryParameterSet("WetTreshold") || !req->getParams()->isQueryParameterSet("Type"))
        {
        ...
        response(res, handlecreate(ModuleId, pinNumber, DryTreshold, WetTreshold, Type));
    }));
    context->getNetwork()->addApi(new ResourceNode(std::string(class_path + "/update"), LambdaResourceNode::REQUEST_METHOD_PUT, [&](HTTPRequest * req, HTTPResponse * res) {
        ...
        if (!req->getParams()->isQueryParameterSet("id") || !req->getParams()->isQueryParameterSet("ModuleId") || !req->getParams()->isQueryParameterSet("PinNumber") || !req->getParams()->isQueryParameterSet("DryTreshold") || !req->getParams()->isQueryParameterSet("WetTreshold") || !req->getParams()->isQueryParameterSet("Type"))
        {
            response(res, 400, MISSING_INPUT_PARAMS_MESSAGE);
            return;
        }
        ...
        response(res, handleupdate(id, ModuleId, pinNumber, DryTreshold, WetTreshold, Type));
    }));
...
String SoilMoistureApis::handlecreate(int ModuleId, int pinNumber, int DryTreshold, int WetTreshold, String Type) {
    SoilMoistureEntity* soilmoistureEntity = new SoilMoistureEntity(ModuleId, pinNumber, DryTreshold, WetTreshold, Type);
...
String SoilMoistureApis::handleupdate(int id, int ModuleId, int pinNumber, int DryTreshold, int WetTreshold, String Type) {
    SoilMoistureEntity* soilmoistureEntity = new SoilMoistureEntity(id, ModuleId, pinNumber, DryTreshold, WetTreshold, Type);
...
String SoilMoistureApis::callFunction(String functionName, std::map<String, String> parameters) {
    if (functionName == "create") {
        return handlecreate(parameters["ModuleId"].toInt(), parameters["PinNumber"].toInt(), parameters["DryTreshold"].toInt(), parameters["WetTreshold"].toInt(), parameters["Type"]);
    }
    if (functionName == "update") {
        return handleupdate(parameters["id"].toInt(), parameters["ModuleId"].toInt(), parameters["PinNumber"].toInt(), parameters["DryTreshold"].toInt(), parameters["WetTreshold"].toInt(), parameters["Type"]);
    }
