...
def get_arduino_cli(dir_path):
    ...
    try:
        ...
        compile_command = ['arduino-cli', 'compile', '--config-file', data['configuration'], '--fqbn', data['board'] , dir_path + '\\.vscode\\.build']
        upload_command = ['arduino-cli', 'upload', '-p', data['port'], '--config-file', data['configuration'], '--fqbn', data['board'], dir_path + '\\.vscode\\.build']
    f.close()
